...
Each class has it's own "compile" method.
We could abstract further to the visitor pattern, to enable extending 
the compiler with multiple passes over the same AST.
|#
...
;; a new AST node.
;; We could add a new class, with a new compiler method, or implement vistor, and
;; only compile that new class.
